...
Потоковые запросы читают данные из [топиков](datamodel/topic.md) {{ ydb-short-name }} и записывают результаты в [топики](datamodel/topic.md) или [таблицы](datamodel/table.md) {{ ydb-short-name }}. Данные в топик могут поступать из внешних систем (например, приложение записывает события через SDK), но сам потоковый запрос работает только с сущностями {{ ydb-short-name }}. Прямое чтение из внешних систем, например из Apache Kafka, или запись в таблицы других баз данных, таких как PostgreSQL, не поддерживается.

### Обогащение из таблиц {{ ydb-short-name }} {#enrichment-ydb-tables}

События из потока можно дополнять данными из [таблиц](datamodel/table.md) {{ ydb-short-name }} в текущей базе данных. Для этого в тексте запроса используется [`JOIN`](../yql/reference/syntax/select/join.md): слева задаётся поток (чтение из топика или промежуточный результат), справа — таблица или подзапрос к таблице в роли справочника. Аналогичный приём применяется при [обогащении данными из S3](../dev/streaming-query/enrichment.md), где справочник подключается через внешнюю таблицу над объектным хранилищем.

### Источники {#sources}

**Топики** — основной источник потоковых данных. Поддерживается чтение из **локальных** [топиков](datamodel/topic.md) текущей базы данных (по имени топика) и из **внешних** топиков {{ ydb-short-name }} — в той же или другой базе, к которым доступ организован через [внешний источник данных](datamodel/external_data_source.md) с типом источника `Ydb`. Запрос читает сообщения из одного или нескольких топиков и обрабатывает их по мере поступления. Данные в топик могут записывать:
...
**Топики** — для передачи результатов другим системам или следующим этапам обработки. Запись поддерживается как в **локальные** [топики](datamodel/topic.md) текущей базы данных, так и во **внешние** топики {{ ydb-short-name }} через [внешний источник данных](datamodel/external_data_source.md) с типом источника `Ydb`.
