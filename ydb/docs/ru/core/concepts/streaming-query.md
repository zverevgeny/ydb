...
**Топики** — основной источник потоковых данных. Поддерживается чтение из **локальных** [топиков](datamodel/topic.md) текущей базы данных и из **внешних** топиков {{ ydb-short-name }} — в другой базе. Запрос читает сообщения из одного или нескольких топиков и обрабатывает их по мере поступления. Данные в топик могут записывать:
...
**Топики** — для передачи результатов другим системам или следующим этапам обработки. Запись поддерживается как в **локальные** [топики](datamodel/topic.md) текущей базы данных, так и во **внешние** топики {{ ydb-short-name }}.
