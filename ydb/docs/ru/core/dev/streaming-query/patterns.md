...
**Внешние топики** — это [топики](../../concepts/datamodel/topic.md) в другой базе {{ ydb-short-name }}, к которым обращение из текущей базы выполняется через [внешний источник данных](../../concepts/datamodel/external_data_source.md) с типом источника `Ydb`. В тексте потокового запроса имя топика указывают вместе с именем источника: `<имя_источника>.<имя_топика>`.

Сначала создайте источник командой [CREATE EXTERNAL DATA SOURCE](../../yql/reference/syntax/create-external-data-source.md) (выполняется в той базе, где будет жить потоковый запрос).
