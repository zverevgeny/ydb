...
enum class EOriginator {
    Tx, //some distributed transaction
    Bg, //som background process
};
...
public:
    ...
    virtual bool IsEqualTo(ILock& lock) { Y_UNUSED(lock); return false; }; //TODO use it for an optimization
    virtual bool IsCompatibleWith(ILock& lock) { Y_UNUSED(lock);  return false; } // = 0; //TODO fixme
