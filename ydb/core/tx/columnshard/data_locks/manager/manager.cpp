...
std::optional<TGuard> TManager::Lock(ILock::TPtr&& lock, const ELockType lockType, ILockAccuired::TPtr&& onAccuired) {
    AFL_VERIFY(lock);
...
std::optional<TString> TManager::IsLocked(const TPortionInfo& portion, const TLockScope& scope) const {
    for (const auto& [_, lockInfo] : Locks) {
...
std::optional<TString> TManager::IsLocked(const TGranuleMeta& granule, const TLockScope& scope) const {
    for (const auto& [_, lockInfo] : Locks) {
