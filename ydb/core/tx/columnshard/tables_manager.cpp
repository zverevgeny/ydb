...
std::optional<std::set<NColumnShard::TSchemeShardLocalPathId>> TTablesManager::ResolveSchemeShardLocalPathIdsOptional(
    ...
    if (!HasTable(internalPathId, true)) {
        YDB_LOG_WARN("",
            {"method", "resolve_ss_path_id"},
            {"internal", internalPathId},
            {"result", "not_found"});
        return std::nullopt;
    }
...
std::optional<TInternalPathId> TTablesManager::ResolveInternalPathIdOptional(
    ...
    if (const auto& internalPathId = GenerationIndex.ResolveLive(schemeShardLocalPathId)) {
        ...
        YDB_LOG_WARN("",
            {"method", "resolve_internal_path_id"},
            {"ssLocal", schemeShardLocalPathId},
            {"result", "not_found"});
...
std::optional<NKikimrTxColumnShard::TTableVersionInfo> TTablesManager::LoadLastTableVersionInfo(
    ...
    if (rowset.EndOfSet()) {
        // The TableVersionInfo row was evicted from nice-db (cold page). Rather than crashing the
        // tablet, return nullopt so the truncate proceeds without SchemaPresetId carry-over.
        // This is a safe degradation: the new table will use the default schema preset.
        YDB_LOG_WARN_COMP(NKikimrServices::TX_COLUMNSHARD, "",
            {"method", "LoadLastTableVersionInfo"},
            {"path_id", pathId},
            {"version", version.DebugString()},
            {"reason", "TableVersionInfo row evicted from nice-db"});
        return std::nullopt;
    }
    NKikimrTxColumnShard::TTableVersionInfo versionInfo;
    if (!versionInfo.ParseFromString(rowset.GetValue<Schema::TableVersionInfo::InfoProto>())) {
        YDB_LOG_WARN_COMP(NKikimrServices::TX_COLUMNSHARD, "",
            {"method", "LoadLastTableVersionInfo"},
            {"path_id", pathId},
            {"version", version.DebugString()},
            {"reason", "failed to parse TableVersionInfo proto"});
        return std::nullopt;
    }
    return versionInfo;
}
...
bool TTablesManager::TryFinalizeDropPathOnComplete(const TInternalPathId pathId) {
    ...
    RebuildReadOnlyTablesSnapshots();
    YDB_LOG_DEBUG("",
        {"method", "TryFinalizeDropPathOnComplete"},
        {"pathId", pathId},
        {"size", Tables.size()});
    return true;
}
...
void TTablesManager::MoveTableProgress(
    ...
    NYDBTest::TControllers::GetColumnShardController()->OnAddPathId(
        TabletId, TUnifiedPathId::BuildValid(internalPathId, newSchemeShardLocalPathId));
}

void TTablesManager::CopyTableProgress(NIceDb::TNiceDb& db, const NOlap::TSnapshot& version,
    const TSchemeShardLocalPathId srcSchemeShardLocalPathId, const TSchemeShardLocalPathId dstSchemeShardLocalPathId) {
    YDB_LOG_CREATE_CONTEXT_COMP(NKikimrServices::TX_COLUMNSHARD,
...
std::vector<TTablesManager::TSchemasChain> TTablesManager::ExtractSchemasToClean() const {
    ...
    for (auto&& i : index.GetVersionedIndex().GetSchemaByVersion()) {
        ...
        if (auto ignoreToVersion = index.MutableVersionedIndex().ExtractIgnoreSchemaVersionFor(i.second->GetIndexInfo().GetVersion())) {
            ...
            if (!index.HasDataWithSchemaVersion(predVersion, i.first) && lastSchemaVersion != i.first) {
                YDB_LOG_WARN("",
                    {"event", "schema_to_remove"},
                    {"reason", "empty"},
                    {"from", i.first});
