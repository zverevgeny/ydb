...
public:
    ...
    bool IsLocked(const std::shared_ptr<NDataLocks::TManager>& dataLocksManager) const {
        for (auto&& f : Futures) {
            for (auto&& p : f.second) {
                if (auto lockInfo = dataLocksManager->IsLocked(*p.second, NDataLocks::EAction::Modify)) { //?
        ...
        for (auto&& i : PreActuals) {
            if (auto lockInfo = dataLocksManager->IsLocked(*i.second, NDataLocks::EAction::Modify)) { //?
                AFL_WARN(NKikimrServices::TX_COLUMNSHARD)("event", "optimization_locked")("reason", *lockInfo);
                return true;
            }
        }
...
public:
    ...
    bool IsLocked(const std::shared_ptr<NDataLocks::TManager>& dataLocksManager) const {
        if (MainPortion) {
            if (auto lockInfo = dataLocksManager->IsLocked(*MainPortion, NDataLocks::EAction::Modify)) { //?)) {
                AFL_WARN(NKikimrServices::TX_COLUMNSHARD)("event", "optimization_locked")("reason", *lockInfo);
                return true;
            }
        }
        ...
        const std::shared_ptr<IStoragesManager>& storagesManager) const
    {
        ...
        for (auto&& i : portions) {
            ...
            if (locksManager->IsLocked(*i, NDataLocks::EAction::Modify)) { //?)) {
