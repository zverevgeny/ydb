...
#include "yql_yt_blocking_queue.h"
...
// metadata. It does not talk to any server: offset management for YT queues is
// done via advance_queue_consumer in TYtTopicClient::CommitOffset.
struct TYtPartitionSession final : public TPartitionSessionControl {
    TYtPartitionSession(ui64 sessionId, const TString& topicPath, ui64 partitionId) {
...
class TYtTopicReadSession final : public IReadSession {
        ...
        while (!EventsQ.IsStopped()) {
            ...
            try {
                ...
                EventsQ.Push(TSessionClosedEvent(EStatus::INTERNAL_ERROR,
                    {NIssue::TIssue(TStringBuilder() << "YT queue pull failed: " << ex.what())}), 0);
                break;
            }
...
class TYtTopicWriteSession final : public IWriteSession, private TContinuationTokenIssuer {
    ...
    void WriteLoop() {
        while (auto maybeMsg = EventsMsgQ.Pop(true)) {
            ...
            try {
                ...
                EventsQ.Push(TSessionClosedEvent(EStatus::INTERNAL_ERROR,
                    {NIssue::TIssue(TStringBuilder() << "YT queue push failed: " << ex.what())}), 0);
                break;
            }
...
class TYtTopicClient final : public ITopicClient {
public:
    explicit TYtTopicClient(const TYtTopicClientSettings& settings)
        : Settings(settings)
    {
    ...
    TAsyncStatus CreateTopic(const TString& path, const TCreateTopicSettings& settings) final {
        Y_UNUSED(settings);
        // A YT queue is an ordered dynamic table that must be created and mounted
        // out of band (or by the control plane). We treat an existing table as
        // success and surface a clear error otherwise.
        try {
            auto exists = NYT::NConcurrency::WaitFor(
                Settings.Client->NodeExists(ResolvePath(path))).ValueOrThrow();
            if (exists) {
                return MakeSuccess();
            }
            return MakeError(EStatus::BAD_REQUEST,
                TStringBuilder() << "YT queue " << path << " does not exist; create the ordered dynamic table first");
        } catch (const std::exception& ex) {
            return MakeError(EStatus::INTERNAL_ERROR, ex.what());
        }
    }
    ...
    TAsyncStatus DropTopic(const TString& path, const TDropTopicSettings& settings) final {
        ...
        try {
            NYT::NConcurrency::WaitFor(
                Settings.Client->RemoveNode(ResolvePath(path))).ThrowOnError();
            return MakeSuccess();
        } catch (const std::exception& ex) {
            return MakeError(EStatus::INTERNAL_ERROR, ex.what());
        }
    ...
    std::shared_ptr<IReadSession> CreateReadSession(const TReadSessionSettings& settings) final {
        ...
        auto partitionSession = MakeIntrusive<TYtPartitionSession>(
            static_cast<ui64>(partitionIndex), topicPath, static_cast<ui64>(partitionIndex));

        return std::make_shared<TYtTopicReadSession>(
            Settings.Client,
            queuePath,
            consumerPath,
            partitionSession,
    ...
    std::shared_ptr<IWriteSession> CreateWriteSession(const TWriteSessionSettings& settings) final {
        ...
        return std::make_shared<TYtTopicWriteSession>(session, nameTable, dataColumnId);
    }
...
private:
    ...
    const TYtTopicClientSettings Settings;
};

} // anonymous namespace

ITopicClient::TPtr CreateYtTopicClient(const TYtTopicClientSettings& settings) {
    return MakeIntrusive<TYtTopicClient>(settings);
}
